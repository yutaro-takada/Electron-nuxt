    ...
    <div class="container"> 
        ...
        <form>
            ...
            <label for="">フリガナ</label><br>
                <input v-model="nameKana"  id="input1-name" type="text" name="furigana" placeholder="ヤマダ タロウ"/><br>
                <span>{{errors.nameKana}}</span><br>
            <label for="">メールアドレス</label><br>
                <input v-model="mail" id="input-mail" type="text" name="email" placeholder="test@test.com"/><br>
                <span>{{errors.mail}}</span><br>
            <label for="">電話番号[ハイフンなし]</label><br>
                <input v-model="phoneNumber" id="input-tel" type="text" name="tel" placeholder="080XXXXXXXX"/><br>
                <span>{{errors.phoneNumber}}</span><br>
            <div class="btn"> 
                <b-button variant="primary" @click="confirm()">入力内容を確認する</b-button><br>
    ...
    watch:{
        ...
        phoneNumber(phoneNumber) {
            if (!phoneNumber || !!phoneNumber.match(/^[0-9\-]+$/)) {
                ...
                this.$set(this.errors, "phoneNumber", '電話番号は半角数字/ハイフンなしで入力してください。')
             }
    },
    },
    methods: {
